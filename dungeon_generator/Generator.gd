...
var room_scene = preload("res://dungeon_generator/Room.tscn")
...
func get_random_habitable_tile():
    var tiles = get_used_cells()
    tiles.shuffle()
    for tile in tiles:
        if get_cell_autotile_coord(tile.x, tile.y) == Vector2(1, 1):
            return tile
            
    return tiles[0]
    
func get_random_unhabitable_tile():
    pass
